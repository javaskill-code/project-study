...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
     ...
     * <p>
     * 与jdk1.8中HashMap中的定义很相似，不过value和next属性用volatile修饰保证了内存可见性，
     * 没有setValue方法直接改变Node的value属性
     */
     ...
     *                        establishing the initial table size  负载因子
     * @throws IllegalArgumentException if the initial capacity of
    ...
    /**
     * {@inheritDoc}
     * 这个table里到底装了多少东西其实是个不确定的数量，因为不可能在调用size()方法的时候像GC的“stop the world”
     * 一样让其他线程都停下来让你去统计，因此只能说这个数量是个估计值。
     */
    public int size() {
        long n = sumCount();
        return ((n < 0L) ? 0 :
     ...
     * <p>
     * ①.如果一个或多个线程正在对ConcurrentHashMap进行扩容操作，当前线程也要进入扩容的操作中。这个扩容的操作之所以能被检测到，、
     * 是因为transfer方法会将已经操作过扩容桶头结点置为ForwardingNode节点，如果检测到需要插入的位置被该节点占有，就帮助进行扩容。
     * ②.如果检测到要插入的节点是非空且不是ForwardingNode节点，就对这个节点加锁，这样就保证了线程安全。
     * <p>
     */
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        //ConcurrentHashMap不允许null作为key和value
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            int n, i, fh;
            // 若tab 为空 初始化tab
            //检查table 是否初始， 没有的话初始化table，重新循环
            if (tab == null || (n = tab.length) == 0)
                ...
                //锁住当前数组i位置上的节点
                synchronized (f) {
                    // 判断是否是节点f是否为当前数组i位置上的节点，防止被其他线程修改
                    if (tabAt(tab, i) == f) {
                        // 当前桶的结构为链表
                        if (fh >= 0) {
                            binCount = 1;
     ...
     * 根据sizeCtl 的值 控制 表的初始化
     */
    private final Node<K, V>[] initTable() {
        Node<K, V>[] tab;
        int sc;
        while ((tab = table) == null || tab.length == 0) {
            //initTable方法判断sizeCtl值，若sizeCtl值为-1即有其他线程正在初始化，调用Thread.yield()让出CPU时间片
            //若sizeCtl<0，即存在其他线程正在初始化操作，确保只有一个线程进行初始化
            if ((sc = sizeCtl) < 0)
                Thread.yield(); // lost initialization race; just spin
                //在初始化的线程通过Unsafe.compareAndSwapInt方法修改sizeCtl为-1，初始化数组和扩容阈值
...
//                            在nextTable的i位置上插入一个链表
    ...
    /**
     * 为了统计元素个数，ConcurrentHashMap通过累加baseCount和CounterCell数组中的数量，
     * 元素个数保存baseCount中，部分元素的变化个数保存在CounterCell数组中，得到元素的总个数。
     *
     * @return
     */
    final long sumCount() {
